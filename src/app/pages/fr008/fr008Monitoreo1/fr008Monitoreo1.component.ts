...
import { Fr008Monitoreo1Model } from '../../../models/Fr008Monitoreo1Model.model'
...
@Component({
    selector: 'fr008Monitoreo1-cmp',
    templateUrl: 'fr008Monitoreo1.component.html',
    styleUrls: ['fr008Monitoreo1.component.css']
})

export class Fr008Monitoreo1 implements OnInit {

    fechaExcel = this.datepipe.transform(new Date(), 'dd-MM-yyyy');

    displayedColumnsFRS: string[] = ['item', 'location', 'supplier', 'fecha_activacion', 'fecha_de_creacion', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus'];
    headFRS = [['item', 'location', 'supplier', 'fecha_activacion', 'fecha_de_creacion', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus']];

    headILExcel = [['XXMAB_FEMSA_REPL_SIV_FR008', '', '', '', '', '', '', 'Fecha creación: ' + this.fechaExcel.toString(), ''],
    ['item', 'location', 'supplier', 'fecha_activacion', 'fecha_de_creacion', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus']]
    ...
    buscar() {
        ...
        if (diasTranscurridos > 30) {
            ...
            if (this.idReferencia != undefined) {
                url = environment.urlServices + 'fr008/xxmab_femsa_repl_siv_fr008/?fechaInicio=' + fInicio + '&fechaFin=' + fFinal + '&idRef=' + this.idReferencia + '&estatusReg=' + estatusBusuqeda;
            } else {
                url = environment.urlServices + 'fr008/xxmab_femsa_repl_siv_fr008/?fechaInicio=' + fInicio + '&fechaFin=' + fFinal + '&idRef=' + '&estatusReg=' + estatusBusuqeda;
            }
    ...
    showTableFRS() {
        this.dataResponseFRS = new MatTableDataSource<Fr008Monitoreo1Model>(this.responseFRS['items'])
    ...
    createPdfFRS() {
        ...
        this.dataResponseTableFRS.forEach(element => {
            var temp = [element.item, element.location, element.supplier, element.fecha_activacion, element.fecha_de_creacion, element.creation_date?.substr(0, 10), element.id_ref, element.last_update_date, element.last_updated_by, element.comentarios, element.estatus];
            rows.push(temp);
        })
        ...
        doc1.setFontSize(18);
        doc1.text('Flujo de información de Recuperación de Ligas de PIEVE a DAS.', 11, 8);
        doc1.setFontSize(11);
    ...
    exportExcelFRS() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "fr008Monitoreo1.xlsx");
        XLSX.writeFile(wb, "fr008Monitoreo1.xlsx");
    ...
    updateEstatusFRS() {
        let fInicio = this.datepipe.transform(this.fechaInicio, 'yyyy-MM-dd');
        let fFinal = this.datepipe.transform(this.fechaFin, 'yyyy-MM-dd');
        this.dialog.open(DialogConfirmFRSFR008, {
...
export class DialogConfirmFRSFR008 {
    constructor(
        public dialogRef: MatDialogRef<DialogConfirmFRSFR008>,
    ...
    updateEstatusFRS(idRef, comentario) {
        ...
        this.http.put<any>(environment.urlServices + 'fr008/xxmab_femsa_repl_siv_fr008/', body, { headers })
            .subscribe(data => {
