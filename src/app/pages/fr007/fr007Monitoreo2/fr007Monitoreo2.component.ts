...
import { Fr007Monitoreo2Model } from '../../../models/Fr007Monitoreo2Model.model'
...
@Component({
    selector: 'fr007Monitoreo2-cmp',
    templateUrl: 'fr007Monitoreo2.component.html',
    styleUrls: ['fr007Monitoreo2.component.css']
})

export class Fr007Monitoreo2 implements OnInit {

    fechaExcel = this.datepipe.transform(new Date(), 'dd-MM-yyyy');

    displayedColumnsRIL: string[] = ['item', 'location', 'primary_repl_supplier', 'status', 'last_update_id', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus'];
    headRIL = [['item', 'location', 'primary_repl_supplier', 'status', 'last_update_id', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus']];

    headILExcel = [['XXMAB_REPL_ITEM_LOC_FR007', '', '', '', '', '', '', 'Fecha creación: ' + this.fechaExcel.toString(), ''],
    ['item', 'location', 'primary_repl_supplier', 'status', 'last_update_id', 'creation_date', 'id_ref', 'last_update_date', 'last_updated_by', 'comentarios', 'estatus']]
    ...
    buscar() {
        ...
        if (diasTranscurridos > 30) {
            ...
            if (this.idReferencia != undefined) {
                url = environment.urlServices + 'fr007/xxmab_repl_item_loc_fr007/?fechaInicio=' + fInicio + '&fechaFin=' + fFinal + '&idRef=' + this.idReferencia + '&estatusReg=' + estatusBusuqeda;
            } else {
                url = environment.urlServices + 'fr007/xxmab_repl_item_loc_fr007/?fechaInicio=' + fInicio + '&fechaFin=' + fFinal + '&idRef=' + '&estatusReg=' + estatusBusuqeda;
            }
    ...
    showTableRIL() {
        this.dataResponseRIL = new MatTableDataSource<Fr007Monitoreo2Model>(this.responseRIL['items'])
    ...
    createPdfRIL() {
        ...
        this.dataResponseTableRIL.forEach(element => {
            var temp = [element.item, element.location, element.primary_repl_supplier, element.status, element.last_update_id, element.creation_date?.substr(0, 10), element.id_ref, element.last_update_date, element.last_updated_by, element.comentarios, element.estatus];
            rows.push(temp);
        })
        ...
        doc1.setFontSize(18);
        doc1.text('Replicas de Ligas Reple R2 de DAS a RMS10.', 11, 8);
        doc1.setFontSize(11);
    ...
    exportExcelRIL() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "fr007Monitoreo2.xlsx");
        XLSX.writeFile(wb, "fr007Monitoreo2.xlsx");
    ...
    updateEstatusRIL() {
        let fInicio = this.datepipe.transform(this.fechaInicio, 'yyyy-MM-dd');
        let fFinal = this.datepipe.transform(this.fechaFin, 'yyyy-MM-dd');
        this.dialog.open(DialogConfirmRIL, {
...
export class DialogConfirmRIL {
    constructor(
        public dialogRef: MatDialogRef<DialogConfirmRIL>,
    ...
    updateEstatusRIL(idRef, comentario) {
        ...
        this.http.put<any>(environment.urlServices + 'fr007/xxmab_repl_item_loc_fr007/', body, { headers })
            .subscribe(data => {
